...
impl MediaPlayer for JRiverPlayer {
    ...
    async fn play_artist(&self, artist: &str) -> Result<()> {
        ...
        // Fix: Use Playback/Play for exact artist playback. Use Action=Replace to clear queue.
        let params = format!("Query=[Artist]=[{}]&Action=1", encoded);
    ...
    async fn play_album(&self, album: &str) -> Result<()> {
        ...
        // Fix: Use Playback/Play for exact album playback. Use Action=Replace to clear queue.
        let encoded = urlencoding::encode(&target);
        // Play the exact album tracks, in order.
        let params = format!("Query=[Album]=[{}]&Action=1", encoded);
        self.send_command("Playback/Play", &params).await?;
        Ok(())
    }
    ...
    async fn play_song(&self, song: &str) -> Result<()> {
        ...
        // Fix: Use Playback/Play for exact song playback. Use Action=Replace to clear queue.
        let params = format!("Query=[Name]=[{}]&Action=1", encoded);
    ...
    async fn play_playlist(&self, playlist: &str, shuffle: bool) -> Result<()> {
        ...
        if let Some(best) = matches.first() {
            if let Some((_, id)) = playlists.iter().find(|(n, _)| n == &best.value) {
                ...
                let params = format!("Playlist={}&PlaylistType=ID&Action=1", id);
